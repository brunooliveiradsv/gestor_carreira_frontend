...
function FormularioMusica({ id, onSave, onCancel }) {
    ...
    if (id) {
      ...
      apiClient
        ...
        .then((resposta) => {
          ...
          // DEBUG: Confirma que as tags foram carregadas para o estado
          console.log('[DEBUG] Tags carregadas para o estado:', nomesDasTags);
        })
        .catch(() =>
  ...
  const handleSubmit = async (e) => {
    ...
    // DEBUG: Mostra o que está prestes a ser enviado
    console.log('[DEBUG] Estado das tags no momento da submissão:', tagsSelecionadas);

    const dadosParaEnviar = { ...dadosForm, tags: tagsSelecionadas };
    
    // DEBUG: Mostra o payload final que será enviado para a API
    console.log('[DEBUG] Payload final a ser enviado:', JSON.stringify(dadosParaEnviar, null, 2));
    ...
    try {
      ...
      mostrarNotificacao(
        erro.response?.data?.mensagem || "Falha ao salvar a música.", "error"
      );
    } finally {
      setCarregando(false);
    }
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        <Autocomplete
          ...
          onChange={(event, newValue) => {
            // DEBUG: Mostra o novo valor sempre que as tags são alteradas
            console.log('[DEBUG] Autocomplete mudou. Novo valor:', newValue);
          ...
          renderInput={(params) => (
            <TextField {...params} variant="outlined" label="Tags" placeholder="Adicione ou crie tags" />
          )}
        />
