...
function FormularioCompromisso({ id, onSave, onCancel }) {
  ...
  useEffect(() => {
    apiClient.get('/api/setlists')
        .then(resposta => {
            setSetlistsDisponiveis(resposta.data);
        })
        .catch(erro => {
            mostrarNotificacao("Não foi possível carregar seus setlists.", "warning");
        });
  ...
  return (
    ...
    <MenuItem key={tipo} value={tipo}>{tipo}</MenuItem>
        ...
        <FormControl fullWidth>
            ...
            <Select name="setlist_id" label="Setlist (Opcional)" value={dadosForm.setlist_id || ''} onChange={handleChange}>
                <MenuItem value=""><em>Nenhum</em></MenuItem>
                {setlistsDisponiveis.map((setlist) => (
                    <MenuItem key={setlist.id} value={setlist.id}>{setlist.nome}</MenuItem>
                ))}
            </Select>
        </FormControl>
