...
function PaginaVitrine() {
  ...
  // --- FUNÇÃO DE BUSCA ENVOLVIDA COM useCallback ---
  const buscarDadosVitrine = useCallback(async (signal) => {
    try {
      setCarregando(true);
      const { data } = await apiClient.get(`/api/vitrine/${url_unica}`, { signal }); // Passa o 'signal' para o axios
      setVitrine(data);
      ...
      if (err.name === 'CanceledError') {
        console.log('Pedido cancelado pelo timeout');
        setErro('O servidor está a demorar muito para responder. Tente novamente mais tarde.');
      } else {
  ...
  }, [url_unica]); // A função será recriada se a url_unica mudar

  useEffect(() => {
    if (url_unica) {
        // --- LÓGICA DE TIMEOUT ADICIONADA ---
        const controller = new AbortController();
        const timeoutId = setTimeout(() => {
            controller.abort(); // Cancela o pedido se demorar mais de 10 segundos
        }, 10000); // 10 segundos

        buscarDadosVitrine(controller.signal);
