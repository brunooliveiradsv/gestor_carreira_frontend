...
import {
  ...
  PlaylistAddCheck as SuggestionIcon, // Ícone para Sugestão
...
function Repertorios() {
  ...
  // States para o modal de sugestão
  const [sugestaoModalAberto, setSugestaoModalAberto] = useState(false);
  ...
  const buscarTags = useCallback(async () => {
    try {
      ...
      mostrarNotificacao(
        "Não foi possível carregar as tags para filtro.",
        "error"
      );
    }
  }, [mostrarNotificacao]);
  ...
  const buscarMusicas = useCallback(async () => {
    ...
    try {
      ...
      mostrarNotificacao(
        "Não foi possível carregar as músicas do repertório.",
        "error"
      );
    } finally {
      setCarregando(false);
  ...
  const handleApagar = async (id) => {
    if (
      window.confirm(
        "Tem certeza que deseja apagar esta música do seu repertório geral?"
      )
    ) {
      try {
        await apiClient.delete(`/api/musicas/${id}`);
        mostrarNotificacao("Música apagada com sucesso!", "success");
        buscarMusicas(); // Atualiza a lista
      } catch (erro) {
  ...
  const handleAbrirModalSugestao = (musica) => {
    setMusicaParaSugerir(musica);
    setSugestaoModalAberto(true);
  };

  const handleSucessoFormulario = () => {
    setModo("lista");
    setMusicaSelecionadaId(null);
  };
    ...
    <Paper elevation={6} sx={{ p: { xs: 2, md: 4 } }}>
      ...
      <Grid container spacing={2} sx={{ mb: 4 }}>
              ...
              renderValue={(selectedIds) => (
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                  {selectedIds.map((id) => {
                    ...
                    return (
                      <Chip key={id} label={tag ? tag.nome : id} size="small" />
                    );
                  })}
                </Box>
              )}
        ...
        >
          <Button
            fullWidth
            variant={filtros.semTocarDesde ? "contained" : "outlined"}
            onClick={() =>
              setFiltros((f) => ({ ...f, semTocarDesde: !f.semTocarDesde }))
            }
          >
            Praticar
          </Button>
        ...
        {musicas.length > 0 ? (
              ...
              >
                <Box>
                  ...
                  <Typography color="text.secondary">
                    {musica.artista} - Tom: {musica.tom || "N/A"}
                  </Typography>
                  <Box
                ...
                <Box>
                  <Tooltip title="Sugerir Melhoria">
                    <IconButton
                      onClick={() => handleAbrirModalSugestao(musica)}
                      color="secondary"
                    >
                      <SuggestionIcon />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="Editar Música (Pessoal)">
                    <IconButton
                      onClick={() => {
                        setMusicaSelecionadaId(musica.id);
                        setModo("editar");
                      }}
                    >
                      <EditIcon />
                  ...
                  <Tooltip title="Apagar Música">
                    <IconButton
                      onClick={() => handleApagar(musica.id)}
                      color="error"
                    >
                      <DeleteIcon />
                    </IconButton>
          ...
          <Grid item xs={12}>
            <Typography sx={{ textAlign: "center", p: 4 }}>
              Nenhuma música encontrada. Que tal adicionar uma nova?
            </Typography>
          </Grid>
        )}
      </Grid>
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      {musicaParaSugerir && (
        <FormularioSugestao
          open={sugestaoModalAberto}
          onClose={() => setSugestaoModalAberto(false)}
          musica={musicaParaSugerir}
        />
      )}
    </Container>
  );
