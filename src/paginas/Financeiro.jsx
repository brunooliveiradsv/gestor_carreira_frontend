  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Paper elevation={6} sx={{ p: { xs: 2, md: 4 }, mb: 4 }}>
        <Typography variant="h4" component="h1" fontWeight="bold" gutterBottom sx={{ color: theme.palette.text.primary }}>
          Painel Financeiro
        </Typography>
        <Grid container spacing={3} sx={{ mb: 3 }}>
          <Grid item xs={12} md={4}>
            <Paper variant="outlined" sx={{ p: 2, display: 'flex', alignItems: 'center', borderColor: theme.palette.divider }}>
          ...
          <Grid item xs={12} md={4}>
            <Paper variant="outlined" sx={{ p: 2, display: 'flex', alignItems: 'center', borderColor: theme.palette.divider }}>
              <Avatar sx={{ bgcolor: saldo >= 0 ? theme.palette.success.light : theme.palette.error.light, mr: 2 }}>
                <AccountBalanceWalletIcon sx={{ color: saldo >= 0 ? theme.palette.success.dark : theme.palette.error.dark }} />
              </Avatar>
              <Box>
                <Typography color="text.secondary">Balanço (período)</Typography>
                <Typography variant="h5" fontWeight="bold" sx={{ color: saldo >= 0 ? theme.palette.success.main : theme.palette.error.main }}>R$ {saldo.toFixed(2)}</Typography>
              </Box>
            </Paper>
          </Grid>
        </Grid>
          ...
          <Grid item xs={12} lg={8}>
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              <ButtonGroup size="small">
                <Button variant={filtroPeriodo === 'mes' ? 'contained' : 'outlined'} color="primary" onClick={() => setFiltroPeriodo('mes')}>Este Mês</Button>
                <Button variant={filtroPeriodo === 'ano' ? 'contained' : 'outlined'} color="primary" onClick={() => setFiltroPeriodo('ano')}>Este Ano</Button>
                <Button variant={filtroPeriodo === 'tudo' ? 'contained' : 'outlined'} color="primary" onClick={() => setFiltroPeriodo('tudo')}>Tudo</Button>
              </ButtonGroup>
              <ButtonGroup size="small">
                <Button variant={filtroTipo === 'todos' ? 'contained' : 'outlined'} color="primary" onClick={() => setFiltroTipo('todos')}>Todos</Button>
                <Button variant={filtroTipo === 'receita' ? 'contained' : 'outlined'} color="primary" onClick={() => setFiltroTipo('receita')}>Receitas</Button>
                <Button variant={filtroTipo === 'despesa' ? 'contained' : 'outlined'} color="primary" onClick={() => setFiltroTipo('despesa')}>Despesas</Button>
              </ButtonGroup>
            </Box>
          </Grid>
      ...
      {mostrarFormulario && (
        <Paper elevation={6} sx={{ p: 3, mb: 4 }}>
          <Box component="form" onSubmit={handleFormSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Typography variant="h6" sx={{ color: theme.palette.text.primary }}>Adicionar Transação Manual</Typography>
            ...
            <Button type="submit" variant="contained" color="primary">Salvar Transação</Button>
          </Box>
        </Paper>
      )}

      <Paper elevation={6} sx={{ p: { xs: 2, md: 4 } }}>
        <Typography variant="h5" component="h2" gutterBottom fontWeight="bold" sx={{ color: theme.palette.text.primary }}>Histórico de Transações</Typography>
        <Box>
          {transacoesFiltradas.length > 0 ? transacoesFiltradas.map(t => (
            <Card key={t.id} sx={{ mb: 1.5, display: 'flex', alignItems: 'center', p: 2 }}>
              <Avatar sx={{ bgcolor: t.tipo === 'receita' ? theme.palette.success.light : theme.palette.error.light, mr: 2 }}>
                {t.tipo === 'receita' ? <TrendingUpIcon sx={{ color: theme.palette.success.dark }} /> : <TrendingDownIcon sx={{ color: theme.palette.error.dark }} />}
              </Avatar>
              <Box sx={{ flexGrow: 1, overflow: 'hidden' }}>
                <Typography variant="body1" fontWeight="bold" noWrap sx={{ color: theme.palette.text.primary }}>{t.descricao}</Typography>
                <Typography variant="body2" color="text.secondary">{new Date(t.data).toLocaleDateString('pt-BR')}</Typography>
              </Box>
              <Typography variant="h6" fontWeight="bold" sx={{ color: t.tipo === 'receita' ? theme.palette.success.main : theme.palette.error.main, minWidth: '120px', textAlign: 'right' }}>
                {t.tipo === 'receita' ? '+' : '-'} R$ {parseFloat(t.valor).toFixed(2)}
              </Typography>
            </Card>
          )) : <Typography sx={{ color: theme.palette.text.secondary }}>Nenhuma transação encontrada para os filtros selecionados.</Typography>}
        </Box>
      </Paper>
    </Container>
  );
