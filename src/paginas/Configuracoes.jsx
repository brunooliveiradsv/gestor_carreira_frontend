...
import {
  ...
  Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Grid, Avatar, Badge, IconButton, Link
} from "@mui/material";
...
function Configuracoes() {
  ...
  // Estados para os campos da vitrine
  ...
  return (
    <Box>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography color="text.secondary">Gerencie suas informações de acesso e perfil público.</Typography>
      </Box>
      ...
      <Paper sx={{ p: { xs: 2, md: 3 }, mb: 4 }}>
        ...
        <Box component="form" onSubmit={handleSalvarPerfilPublico} noValidate sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 2 }}>
            <TextField label="URL Única" helperText="Ex: o-nome-da-sua-banda" value={urlUnica} onChange={(e) => setUrlUnica(e.target.value.toLowerCase().replace(/\s+/g, '-'))} fullWidth />
            <TextField label="Biografia" multiline rows={4} value={biografia} onChange={(e) => setBiografia(e.target.value)} fullWidth />
            ...
            <TextField 
              ...
              InputProps={{ startAdornment: <Instagram sx={{ mr: 1, color: 'text.secondary' }} /> }}
            />
            <TextField 
              label="YouTube" 
              value={links.youtube} 
              onChange={(e) => handleLinkChange('youtube', e.target.value)}
              InputProps={{ startAdornment: <YouTube sx={{ mr: 1, color: 'text.secondary' }} /> }}
            />
            <TextField 
              label="Spotify" 
              value={links.spotify} 
              onChange={(e) => handleLinkChange('spotify', e.target.value)}
              InputProps={{ startAdornment: <MusicNote sx={{ mr: 1, color: 'text.secondary' }} /> }}
            />
            ...
            <Grid>
              <Paper sx={{ p: { xs: 2, md: 3 }}}>
                ...
                <Box component="form" onSubmit={abrirDialogoSenha} noValidate sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 2 }}>
                  ...
                  <TextField label="Confirmar Nova Senha" type="password" value={confirmarNovaSenha} onChange={(e) => setConfirmarNovaSenha(e.target.value)} fullWidth />
