...
function EditorDeSetlist() {
  ...
  const buscarDados = useCallback(async () => {
    try {
      ...
      const idsNoSetlist = new Set(
        (setlistRes.data.musicas || []).map((m) => m.id)
      );
      setRepertorioGeral(
        repertorioRes.data.filter((m) => !idsNoSetlist.has(m.id))
      );
    } catch (error) {
  ...
  const onDragEnd = (result) => {
    ...
    if (
      source.droppableId === "setlist" &&
      destination.droppableId === "setlist"
    ) {
                      ...
                      >
                        {(provided) => (
                          <ListItem
                            ...
                            secondaryAction={
                              // Adicionado para o botão "Adicionar"
                              <Tooltip title="Adicionar ao Setlist">
                                <IconButton
                                  edge="end"
                                  aria-label="add"
                                  onClick={() => adicionarAoSetlist(musica)}
                                >
                                  <PlaylistAddIcon />
                                </IconButton>
                              </Tooltip>
                ...
                {(provided) => (
                  <List {...provided.droppableProps} ref={provided.innerRef}>
                    {musicasNoSetlist.map((musica, index) => (
                      <Draggable
                        key={`setlist-${musica.id}`}
                        draggableId={`setlist-${musica.id}`}
                        index={index}
                      >
                        {(provided) => (
                          <ListItem
