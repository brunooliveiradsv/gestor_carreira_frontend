// ficheiro: /api/musicas/busca-inteligente.js (VERSÃO FINAL COM URLS CORRIGIDAS)
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (resultadoBuscaGoogle.data.items && resultadoBuscaGoogle.data.items.length > 0) {
            const itemCifraClub = resultadoBuscaGoogle.data.items.find(item => item.link.includes('cifraclub.com.br'));
        ...
        console.error("[Detetive Final] ERRO CRÍTICO:", error.response ? JSON.stringify(error.response.data) : error.message);
